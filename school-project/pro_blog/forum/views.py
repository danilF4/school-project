...
def post_update(request, pk):
	# put the decorator for post's user
	all_subjects 	= Subject.objects.all()
	grades 			= GradePost.objects.all()
...
def detail2(request, pk, slug):
	...
	grades			= GradePost.objects.all()
	query 	        = request.GET.get('q')
	#print(instance.comments.first().total_comment_likes())
	...
	if form.is_valid():
		...
		return HttpResponseRedirect(instance.get_absolute_url())
	args 			= {'post':instance, 'all_subjects':all_subjects, 'grades':grades, 'comments':comments, 'form':form}
...
def unanswered(request):
	...
	posts 			= Post.objects.filter(timestamp__gte=three_days_ago, right_answer=False).order_by('-timestamp')
	query 	        = request.GET.get('q')
	if query:
		posts = posts.filter(
			Q(title__icontains=query)|
