...
def make_right(request, pk, pk_comment):
	...
	if instance.right_answer == False:
		...
		if not comment.user == instance.user:
			user.reputation += 15
			user.save()
			return redirect('/forum/post/{}/{}/'.format(instance.pk, instance.slug))
...
@login_required()
def mark_comment(request, pk):
	comment 			 = get_object_or_404(Comment, pk=pk)
	post 				 = get_object_or_404(Post, pk=comment.related_post.pk)
	comments 			 = Comment.objects.filter(related_post=post.pk)
	comment_right_answer = related_comments.filter(right_answer=True).first()
	if post.right_answer:
		if request.user == comment_right_answer.user:
			post.right_answer    = False
			comment_right_answer = False
			comment_right_answer.save()
			post.save()
		else:
			comment_right_answer.right_answer = False
			comment.right_answer   			  = True
			comment.save()
			comment_right_answer.save() 
	if post.right_answer == False:
		comment.right_answer = True
		post.right_answer 	 = True
		comment.save()
		post.save()
...
def like_post_redirect(request, pk, slug, user_pk):
	...
	return redirect('/forum/post/{}/{}/'.format(post.id, post.slug))
